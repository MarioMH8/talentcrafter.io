...
import { UserEmailMother, UserMother, UserPasswordMother } from '@talentcrafter/user/mother/domain';
...
describe('User', () => {
	...
	test('should create a User with specific password', () => {
		const userPassword = UserPasswordMother.random();
		const user = UserMother.create({
			password: userPassword.value,
		});

		expect(user.password?.value).toBe(userPassword.value);
	});
	...
	describe('create', () => {
		test('should create a new User', () => {
			...
			expect(created.password?.value).toBe(parameters.password);
	...
	describe('update', () => {
		test('should update a User', () => {
			...
			expect(updated.password?.value).toBe(user.password?.value);
	...
	describe('updatePassword', () => {
		test('should update a User password', () => {
			const user = UserMother.random();
			const newPassword = UserPasswordMother.random();
			const updater = 'updater-id';

			const result = User.updatePassword(user, newPassword.value, updater);
			...
			expect(updated.password?.value).toBe(newPassword.value);
			expect(updated.createdBy.value).toBe(user.createdBy.value);
			expect(updated.createdAt.value).toBe(user.createdAt.value);
			expect(updated.updatedBy.value).toBe(updater);
			expect(updated.updatedAt.value).toStrictEqual(date);
			...
			const events = updated.pullDomainEvents();
			expect(events.length).toBe(1);
			expect(events[0]?.eventName).toBe('user.password-updated');
		});
	});
